...
public class LocalesServiceBean implements IServiceBean {
    public static final String[] WHERE_PROPERTIES = {"active"};

    private final IGenericService<Locale, Long> service;
    private final String sub_path;
    private final java.util.Locale default_locale;
    ...
    public LocalesServiceBean(IGenericService<Locale, Long> service, java.util.Locale default_locale, java.util.Locale locale, String sub_path, boolean only_active) {
        ...
        this.sub_path = "/".equals(sub_path) ? "/index.htm" : sub_path;
    ...
    public Object getData() {
        List<String> locales;
        if (only_active) {
            locales = (List<String>) service.getSingleProperty("name", WHERE_PROPERTIES, new Object[]{Boolean.TRUE}, 0, 0, LocaleConfig.ORDER_BY, LocaleConfig.ORDER_HOW);
        } else {
            locales = (List<String>) service.getSingleProperty("name", null, null, 0, 0, LocaleConfig.ORDER_BY, LocaleConfig.ORDER_HOW);
        }

        if (locales == null) {
            return null;
        } else {
            List<Locale> rez = new ArrayList<Locale>(locales.size() + 1);
            for (String locale1 : locales) {
                Locale tmp = new Locale();
                tmp.setName(locale1);
