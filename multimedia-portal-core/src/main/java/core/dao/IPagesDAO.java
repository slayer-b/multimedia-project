 ...
 */
public interface IPagesDAO<T extends Pages, ID extends Serializable> extends IGenericDAO<T, ID>{
	/**
	 ...
	 */
	List<T> getPagesForRelocateOrdered(Long id, String[] orderBy, String[] orderHow);
	 ...
	 * @param orderBy properties to order
	 * @param propertyNames is applied as where condition on all children
	 * @param propertyValues is applied as where condition on all children
	 * @param orderHow type of sorting ASC or DESC
     ...
     */
    boolean checkRecursion(Long id,Long newIdPages);
