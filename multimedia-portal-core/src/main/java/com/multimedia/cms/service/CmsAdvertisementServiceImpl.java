 ...
 */
@Service("cmsAdvertisementService")
public class CmsAdvertisementServiceImpl extends AGenericCmsService<Advertisement, Long>{
	...
	private static final List<String> CMS_SHORT_ALIAS = Arrays.asList("id","name","sort","active","position");
	...
	@Override
	public Advertisement getInsertBean(Advertisement obj) {
	    Advertisement a;
		if (obj == null) {
			a = new Advertisement();
		} else {
	...
	public void initUpdate(Map<String, Object> model) {
		model.put("positions", this.advertisementService.getPositions());
	}
...
//------------------------------------ initialization ----------------------------------
	...
	public void setService(IGenericService<Advertisement, Long> value) {
		if (value instanceof IAdvertisementService){
			this.advertisementService = (IAdvertisementService)value;
		}
