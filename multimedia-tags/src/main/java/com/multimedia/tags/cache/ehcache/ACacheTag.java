...
public abstract class ACacheTag extends BodyTagSupport {
    private static final long serialVersionUID = -9041938181888536516L;

    private final Logger logger = LoggerFactory.getLogger(ACacheTag.class);


    private static final String DEFAULT_GENERATION_TEXT = "Генерируется";
    private String region_key;

	/* the text that will be shown while cached content is generated */
    private String generation_text = DEFAULT_GENERATION_TEXT;

    private Object element_key;
    ...
    public int doStartTag() throws JspException {
        ...
        synchronized (this.getClass()) {
			...
			if (cache==null) {
				logger.warn("specified cacheManager has no region '{}', using default", region_key);
				getCacheManager().addCache(region_key);
				cache = getCacheManager().getEhcache(region_key);
			}
