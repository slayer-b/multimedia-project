    ...
    <table align="center" border="1" cellpadding="2" cellspacing="0" style='border-collapse: collapse;'>
        ...
        <c:forEach items="${content_data}" var="row" varStatus="i">
            ...
            <tr class="${tr_class}">
                ...
                <td>${row.id}</td>
                <td>
                    <div>${row.blockLocation.location_type}</div>
                    <c:if test="${row.blockLocation.location_type eq 'SITE'}">${row.blockLocation.siteUrl}</c:if>
                </td>
                <td>
                    <div>${row.content_type}</div>
                    <div>просмотры:${row.blockLocation.stats.views}</div>
                    <div>клики:${row.blockLocation.stats.clicks}</div>
                </td>
                <td align="center">
