...
<script type="text/javascript">
    ...
    var prefixes = new Array("id.", "sort.", "active.", "_active.", "position.");
    var count = 0;
</script>
    ...
    <table align="center" border="1" cellpadding="2" cellspacing="0" style='border-collapse: collapse;'>
        ...
        <c:forEach items="${content_data}" var="row" varStatus="i">
            ...
            <tr class="${tr_class}">
                ...
                <td width="100px">
                    <select id="position.${i.index}" name="data[${i.index}].position">
                        <c:forEach items="${positions}" var="position">
                        <option<c:if test="${position eq row.position}"> selected</c:if>>${position}
                            </c:forEach>
                    </select>
                </td>
                ...
                <td align="center">
                    ...
                    <a href="../advertisementPages/index.htm?id_advertisement_nav=${row.id}"
                       title="разрешения/запрещения для страниц">разрешения</a>
                </td>
            </tr>
